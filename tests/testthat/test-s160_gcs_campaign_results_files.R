test_that("strips campaign prefix from filenames", {
  ...
  expect_equal(s160_gcs_campaign_results_files(1980),
               c("1980_raw_data_download.csv", "1980_summary.csv"))
})
...
test_that("wraps GCS list error with campaign context", {
  ...
  expect_error(s160_gcs_campaign_results_files(1980),
               "Failed to list files for campaign 1980.*auth expired")
})
