...
- **Campaign metrics** -- download the consolidated per-campaign metrics + recipient-latency projection (`campaign_all.parquet`) with `campaign_metrics_pull()`, read it into analysis-ready cells with `campaign_metrics_records()`, and roll those up to funnel counts and rates by any project dimension with `campaign_metrics_summary()`.
...
The pure entry points `latency_run()` and `disposition_run()` take an in-memory data frame and return a list with a `consolidated` data frame plus provenance `meta`. The disposition readers (`disposition_summary()` / `disposition_records()` / `disposition_screen()`) read the Parquet projection and return a data frame, and `disposition_pull()` downloads that projection from GCS and returns a local path. The opt-out readers mirror that pair: `opt_out_pull()` downloads the opt-out list and `opt_out_screen()` annotates a sample with each number's opt-out status. `campaign_metrics_pull()` downloads the consolidated per-campaign metrics + recipient-latency projection; `campaign_metrics_records()` reads it into analysis-ready cells and `campaign_metrics_summary()` rolls those up to funnel counts and rates by any project dimension (it is aggregate metrics, not phone-keyed PII, so there is no screen helper). Fleet orchestration and Parquet persistence live in downstream consumer projects. See `?survey160r` for an overview from the R console.
