...
R client for Survey160 data. Reads campaign results from Google Cloud Storage, triggers fresh exports via the API, and computes per-campaign recipient-latency reports written as Parquet to GCS.

## Stack

- R 4.x, packaged for R-universe.
- Imports: googleCloudStorageR, httr, jsonlite, arrow, lubridate, yaml, digest, dplyr, rlang.
- Suggests: testthat (edition 3), mockery, withr, lintr, covr, duckdb, DBI, askpass.
...
- `scripts/` -- maintenance scripts (e.g. `bulk_reprocess.R`); excluded from the built tarball via `.Rbuildignore`.
