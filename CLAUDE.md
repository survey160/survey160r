...
- **Bare name = pure compute, or reads/fetches a survey160r-derived artifact.** Everything a user calls: the `latency_*`, `disposition_*`, `opt_out_*`, `question_*`, `campaign_metrics_*`, `utils_*`, and `parse_campaign_timestamps` surface. (`utils_*` is the domain for standalone, source-independent helpers -- e.g. `utils_fix_double_utf8`, which repairs double-encoded text in an export -- kept out of a data-domain so they group as general utilities; it lives in `R/utils.R`, distinct from the internal-only `R/aaa_utils.R`.) The pure ones (no I/O) are `latency_run`, `latency_report`, `latency_build_config`, `latency_validate_config`, `latency_config_hash`, `latency_discover_questions`, `latency_funnel`, `disposition_run`, the `*_input_columns` pair, and the campaign-export helpers `question_funnel` / `question_timestamps` / `parse_campaign_timestamps`. The *readers* stay bare even though they do I/O, because they read artifacts survey160r itself produces (a *derived* artifact, not a raw source): the disposition readers `disposition_summary` (one row per phone) / `disposition_records` (one row per `(phone, campaign_id)`) / `disposition_screen` (annotate a sample) read the projection Parquet and `disposition_pull` fetches that projection from GCS; `opt_out_screen` / `opt_out_pull` mirror that pair for the opt-out list; and `campaign_metrics_pull` fetches the consolidated campaign-metrics projection.
