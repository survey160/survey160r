...
- **`s160_` = accesses a raw Survey160 data source.** Reaches Survey160's own campaign data: the REST API (`s160_api_*`), GCS (`s160_gcs_*`), or a raw campaign-export CSV (`s160_read_csv`, `s160_csv_header`). Non-deterministic, mocked in tests, fails on the environment (network, auth, missing file); the boundary where raw data (and, for the remote sources, credentials) enters the process. Building blocks for the pipeline and power users, not the higher-level surface. `s160_datasets()` also carries the prefix: it names WHERE data comes from -- the catalog of raw sources (which datasets and environments exist) -- and although it reads a compiled registry purely today, it becomes a live Survey160 fetch once that registry moves server-side, so the prefix is forward-consistent.
