...
* **`disposition_run()` now emits a `carrier` column, and `disposition_records()`
  ...
  it. It is carried through verbatim (never normalized or MVNO-folded), matched
  case-insensitively, blank/whitespace normalized to `NA`, and `NA` for a campaign
...
* **`utils_fix_double_utf8()`: detect or repair double-encoded (mojibake) text in a
  ...
  neither a dry run nor a repair is silent. The result is returned invisibly, so
  a bare console check prints only the summary line and not the (often
  100-plus-column) data. It is a post-hoc remedy for exports produced before the
  double encoding is fixed at its upload source.
